...
public class JugadorNegro{
    ...
    public ArrayList<ArrayList<Integer>> posPiezasAcomer(ArrayList<ArrayList<String>> tablero,int posx,int posy){
        ...
        if(posy>=2 &&  posx>=2){
             if(tablero.get(posx-1).get(posy-1).compareTo("B")==0 && tablero.get(posx-2).get(posy-2).compareTo("0")==0){
      ...
      public ArrayList<ArrayList<String>> comer(ArrayList<ArrayList<String>> matrix,int posPiesax,int posPiesay,int pcx,int pcy){
       ...
       public ArrayList<ArrayList<String>> comerDama(ArrayList<ArrayList<String>> matrix,int posPiesax,int posPiesay,int pcx,int pcy){
           ...
           // matrix.get(posPiesax).set(posPiesay, "0");
              if(matrix.get(pcx+1).get(pcy+1).compareTo("B")==0){
                    matrix.get(pcx+1).set(pcy+1, "0");
              }else{
                   if(matrix.get(pcx+1).get(pcy-1).compareTo("B")==0){
                         matrix.get(pcx+1).set(pcy-1, "0");
        ...
        for(int i=0; i<listaIni.size();i++){
            ...
            for(int j=0;j<listaPosFinal.size();j++){
                ...
                if(x-a==1 || a-x==1 ){
                    ...
                    ArrayList<ArrayList<String>> matrixMax=simularRespuesta(matrixAux);//Jugador Blanco
        ...
        for(int i=0; i<listaIni.size();i++){
            ...
            for(int j=0;j<listaPosFinal.size();j++){
                ...
                if(x-a==1 || a-x==1 ){
                    ...
                    ArrayList<ArrayList<String>> matrixMin=moverPiezaBlanca(aux,x,y,a,b);
                    ...
                    ArrayList<ArrayList<String>> matrixMin=comerPiezaBlanca(aux,x,y,a,b);
                    int puntaje=obtenerPuntaje(matrixMin);
                    if(puntaje < bandera){
                        res=matrixMin;
                        bandera=puntaje;
    ...
    public ArrayList<ArrayList<Integer>> posicionesIrBlancas(ArrayList<ArrayList<String>> matrix,int i,int j){
        ArrayList<ArrayList<Integer>> listaFinal = new ArrayList<ArrayList<Integer>> ();
    ...
    public ArrayList<ArrayList<Integer>> fichasLibresBlancas(ArrayList<ArrayList<String>> matrix){
        ArrayList<ArrayList<Integer>> listaFinal = new ArrayList<ArrayList<Integer>> ();
        for(int i=0;i<matrix.size();i++){
            for(int j=0;j<matrix.get(i).size();j++){
                if(obtenerFicha(matrix,i,j).compareTo("B")==0 || obtenerFicha(matrix,i,j).compareTo("DB")==0){
                    ArrayList<ArrayList<Integer>> listaPos = posicionesIrBlancas(matrix,i,j);
                    if(listaPos.size()>0){                       
                        ArrayList<Integer> listaAux = new ArrayList<Integer> ();
                        listaAux.add(i);listaAux.add(j);listaFinal.add(listaAux);
                    }
    ...
    public ArrayList<ArrayList<String>> moverPiezaBlanca(ArrayList<ArrayList<String>> matrixE, int x,int y, int a, int b){
        ArrayList<ArrayList<String>> matrix= matrixE;
        String ficha=obtenerFicha(matrix,x,y);
        if(a==7){
            ArrayList<String> lista1=matrix.get(x);lista1.set(y,"0");
    ...
    public ArrayList<ArrayList<String>> comerPiezaBlanca(ArrayList<ArrayList<String>> matrixE, int x,int y, int a, int b){
        ArrayList<ArrayList<String>> matrix= matrixE;
        String ficha=obtenerFicha(matrix,x,y);
        if(x-a==2 && y-b== 2){
            ArrayList<String> lista1=matrix.get(x);lista1.set(y,"0");
